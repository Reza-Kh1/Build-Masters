...
export default function Cards({ props }: { props: PostType[] }) {
    ...
    <section key={index}>
      ...
      >
        <div className="w-2/5 lg:w-1/3 order-1">
          <ImgTag alt={i.image} height={200} src={i.image} width={450} className="w-44 md:w-80 md:h-56 lg:w-[450px] lg:h-56 h-36 object-cover rounded-md shadow-md" />
        </div>
        <div className="w-3/5 lg:w-2/3 order-2 flex justify-evenly md:justify-between flex-col md:py-2 gap-1">
          ...
          <h3 className="text-sm lg:text-lg text-c-blue font-bold dark:text-h-dark cutline cutline-1">{i.name}</h3>
          ...
          <div className="hidden md:!block">
            <button type="button" title={i.name} className="p-[6px] text-xs md:text-base lg:p-2 custom-button-card relative flex items-center bg-gradient-to-b from-orange-300 dark:to-zinc-900 dark:from-slate-600 dark:group-hover:to-zinc-900 dark:group-hover:from-slate-600 to-orange-600 rounded-full group-hover:from-orange-500/70 shadow-md  group-hover:to-orange-600/90 text-white">
